...
const createInvoice = async (req, res) => {
  try {
    ...
    console.log('Invoice response:', createdInvoice);
...
const handlePaymentWebhook = async (req, res) => {
  try {
    ...
    if (event_type === 'invoice.paid') {
      const invoice = data;
      const userId = invoice.externalID.split('_')[1]; // Ambil user ID
