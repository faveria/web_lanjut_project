/**
 * Mobile Blocker Middleware
 * Blocks mobile device access to the application with server-side detection
 */
 ...
 */
const mobileBlocker = (req, res, next) => {
  // Skip mobile blocking for API routes
  if (req.path.startsWith('/api/')) {
    return next();
  }
  
  // Skip mobile blocking for static assets
  if (req.path.startsWith('/static/') || req.path.includes('.')) {
    return next();
  }
  
  // Skip mobile blocking for the mobile not supported page
  ...
  if (isLikelyMobile(req)) {
    // Send the mobile not supported HTML page
    return res.status(403).sendFile('mobile-not-supported.html', { root: 'src/public' });
  }
  
  next();
};

module.exports = mobileBlocker;